...
districts <- districts |>
  ...
  ungroup()
g1 <- ggplot() +
  geom_spatvector(data = districts, aes(fill = log(length)), color = NA) +
...
g2 <- ggplot() +
  geom_spatvector(data = districts, aes(fill = log(expectedlength)), color = NA) +
  scale_fill_distiller("Length (log)", palette = "Spectral", direction = 1) +
  facet_wrap(~wave) +
  theme_bw() +
...
#| crop: true
ggplot() +
  geom_spatvector(data = districts, aes(fill = log(length) - log(expectedlength)), color = NA) +
  scale_fill_distiller("Length (log)", palette = "Spectral", direction = 1) +
  facet_wrap(~wave) +
  theme_bw() +
