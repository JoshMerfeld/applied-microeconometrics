...
**Due date: Thursday, September 23rd before class**

The goal of this assignment is just to get you coding in R (well, RStudio) and R Markdown. You will use data from the Penn World Tables (information about the latest release [here](https://www.rug.nl/ggdc/productivity/pwt/?lang=en)).^[You'll likely want to use this website to figure out what all the variables are!] I have uploaded an Excel file -- called pwt1001 -- to the assignment 1 folder with three tabs:
...
You can load Excel data into R with the `readxl` package, specifically with the function `readexcel`. [Here](https://github.com/rstudio/cheatsheets/blob/main/data-import.pdf) is a handy cheatsheet on importing different types of data into R. Note that you will need to specify which tab in the Excel file you want to load into R!

I would like you to do the following:

- Create a figure with log expenditure-side GDP per capita (per person) on the y-axis and year on the x-axis. How you do this is up to you, but I'd suggest something along the lines of `geom_smooth` and `ggplot2` (you can find a handy cheatsheet for ggplot2 [here](https://www.maths.usyd.edu.au/u/UG/SM/STAT3022/r/current/Misc/data-visualization-2.1.pdf)).
- Create a new variable called "decade" that is equal to the decade of the year variable. For example, 1960 would be 1960-1969, 1970 would be 1970-1979, etc. You can do this with the `cut` function, but there are many other ways to do it, too. Please do 1970 through 2019 (i.e. 1970-1979, 1980-1989, etc.) only. You can drop the other years.
  - Create a table with mean expenditure-side GDP p.c., population, average hours worked, and *one variable of your choice* by decade.
